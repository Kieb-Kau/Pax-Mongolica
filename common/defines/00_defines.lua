...
NDiplomacy = {
	...
	BASE_BOOST_PARTY_POPULARITY_DAILY_PP = 0.2, 	-- Daily pp cost for boost party popularity
	...
	TENSION_TIME_SCALE_START_DATE = "1933.1.1.12",	-- Starting at this date, the tension values will be scaled down (will be equal to 1 before that)
	...
	RESOURCE_SENT_AUTONOMY_DAILY_FACTOR = 0.0001,	-- If puppet provides resources to its master they increasy their autonomy by the resources factored by this
	WAR_SCORE_AUTONOMY_BASE = 0.0,					-- Value added if any war score is contributed by puppet
	WAR_SCORE_AUTONOMY_FACTOR = 0.5,				-- If puppet generates war score it get a boost to independence 
	...
	AUTONOMY_LEVEL_CHANGE_PP_ANNEX = 1000,			-- Annexation cost
	AUTONOMY_LEVEL_CHANGE_PP_FREE = 400,			-- Break free cost
	...
	CAPITAL_CAPITULATE_BONUS_SCORE = 150,			-- extra bonus when deciding who to capitulate to (applied to capital holder) NEW MTG
...
NCountry = {
	...
	BASE_STABILITY_WAR_FACTOR = 0.0,				-- Default stability war factor
...
NMilitary = {
	...
	PLAN_SPREAD_ATTACK_WEIGHT = 4.0,   -- Was 13.0,/NeuGemi	-- The higher the value, the less it should crowd provinces with multiple attacks.
...
NAir = {
	...
	NON_CORE_STRATEGIC_IMPACT = 0.5,                                -- multiplier for strategic impact of non-core bombing									-- Scale of the selected priority for infastryctyre
...
NAI = {
	...
	PLAN_MIN_SIZE_FOR_FALLBACK = 500,			-- A country with less provinces than this will not draw fallback plans, but rather station their troops along the front
	...
	RESERVE_TO_COMMITTED_BALANCE = 0.05,			-- How many reserves compared to number of committed divisions in a combat (1.0 = as many as reserves as committed)
	DIPLOMACY_COMMUNIST_NOT_NEIGHBOUR = -10,	-- Communists want to stay consolidated with their influence
	MAIN_ENEMY_FRONT_IMPORTANCE = 5.0,			-- How much extra focus the AI should put on who it considers to be its current main enemy.
	EASY_TARGET_FRONT_IMPORTANCE = 6.0,			-- How much extra focus the AI should put on who it considers to be the easiest target.
	...
	ORG_UNIT_NORMAL = 0.55,						-- Organization % for unit to be considered normal
	...
	MANPOWER_FREE_USAGE_THRESHOLD = 1,	--500000			-- If AI has this much manpower he doesn't care about the percentage
	MANPOWER_RESERVED_THRESHOLD = 0,					-- The AI will not deploy more units if he goes below this percentage
	START_TRAINING_EQUIPMENT_LEVEL = 0.8,  --0.9             -- ai will not start to train if equipment drops below this level
	STOP_TRAINING_EQUIPMENT_LEVEL = 0.7,  --0.8              -- ai will not train if equipment drops below this level
	...
	REGION_CONVOY_DANGER_DAILY_DECAY = 2,				-- When convoys are sunk it generates threat in the region which the AI uses to prio nalval missions		-- When convoys are sunk it generates threat in the region which the AI uses to prio nalval missions
	...
	FALLBACK_LOSING_FACTOR = 0.1,						-- The lower this number, the longer the AI will hold the line before sending them to the fallback line
	...
	MIN_FIELD_STRENGTH_TO_BUILD_UNITS = 0.5, --0.7			-- Cancel unit production if below this to get resources out to units in the field
	MIN_MANPOWER_TO_BUILD_UNITS = 0.5,	--0.7				-- Cancel unit production if below this to get resources out to units in the field
	...
	MAX_SCREEN_TASKFORCES_FOR_CONVOY_DEFENSE_MAX_CONVOY_THREAT = 1000,-- AI will increase screen assignment for escort missions as threate increases
	...
	MAX_SCREEN_TASKFORCES_FOR_MINE_SWEEPING_PRIO_MAX_MINES = 500, -- highest mines for highest prio for mine missions
	...
	FUEL_RATIO_TO_EXIST_FUEL_SAVING_MODE = 0.50, 					-- countries will exit fuel saving mode if they have more fuel ratio than this
	...
	WANTED_MAX_FUEL_BUFFER_IN_DAYS_FOR_ARMY_MAX_CONSUMPTION = 180,  -- AI will try to buffer at least this amount of days on max consumption, will trade if necesarry and will go into fuel saving mode/aggresive mode using this buffer 
	WANTED_MAX_FUEL_BUFFER_IN_DAYS_FOR_AIR_MAX_CONSUMPTION  = 180,  -- AI will try to buffer at least this amount of days on max consumption, will trade if necesarry and will go into fuel saving mode/aggresive mode using this buffer
	WANTED_MAX_FUEL_BUFFER_IN_DAYS_FOR_NAVY_MAX_CONSUMPTION = 180,  -- AI will try to buffer at least this amount of days on max consumption, will trade if necesarry and will go into fuel saving mode/aggresive mode using this buffer
...
NOperatives = {
	AGENCY_CREATION_DAYS = 120,						-- Number of days needed to create an intelligence agency
	AGENCY_UPGRADE_DAYS = 45,						-- Number of days needed to upgrade an intelligence agency
	...
	AGENCY_AI_PER_UPGRADE_FACTORIES = 7.2,			-- Used by AI to pace the upgrades. Formula : if( AGENCY_AI_BASE_NUM_FACTORIES <= num_civ_factories - num_upgrades * AGENCY_AI_PER_UPGRADE_FACTORIES )
	AGENCY_UPGRADE_PER_OPERATIVE_SLOT = 7,			-- Number of upgrade needed to unlock an additional operative slot
	...
	BECOME_SPYMASTER_PP_COST = 75,					-- Number of political power used to become Spy Master
